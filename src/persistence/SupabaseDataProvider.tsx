...
export function SupabaseDataProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<DataStore>(() => ({ ...state, repositoryStatus: status === 'ready' && !workspaceId ? 'loading' : status, repositoryError: error, retryRepository: () => { if (user) void load(user) },
    ...
    async updateReminderSettings(changes){
      const{client,workspaceId}=requireContext()
      const saved=await mutate<Tables['workspaces']['Row']>(()=>client.from('workspaces').update({reminder_email_enabled:changes.emailEnabled,reminder_primary_hours:changes.primaryLeadHours,reminder_secondary_enabled:changes.secondaryEnabled,reminder_secondary_hours:changes.secondaryLeadHours}).eq('id',workspaceId).select().single())
      // Keep the current screen mounted: saving preferences does not require bootstrapping the workspace again.
      setState(current=>({...current,reminderSettings:{...current.reminderSettings,emailEnabled:saved.reminder_email_enabled,primaryLeadHours:saved.reminder_primary_hours,secondaryEnabled:saved.reminder_secondary_enabled,secondaryLeadHours:saved.reminder_secondary_hours}}))
    }, async cancelAppointmentReminders(){throw new Error('Los recordatorios se cancelan desde el backend al cambiar la cita.')}, async markReminderSent(){throw new Error('El estado de entrega solo puede cambiarlo el backend.')},async markReminderFailed(){throw new Error('El estado de entrega solo puede cambiarlo el backend.')},
