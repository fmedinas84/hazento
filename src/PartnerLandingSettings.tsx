...
export function PartnerLandingSettings({ profile, entitlementMode, onUpgrade, notify }: Props) {
  ...
  return <div className="partner-settings">
    ...
    {previewOpen && <div className="partner-preview-overlay" role="dialog" aria-modal="true" aria-labelledby="partner-preview-title"><div className="partner-preview-dialog"><button className="partner-preview-close" type="button" aria-label="Cerrar vista previa" onClick={() => setPreviewOpen(false)}>×</button>{photoUrl ? <img src={photoUrl} alt=""/> : <div className="partner-preview-placeholder">Tu foto aparecerá aquí</div>}<div className="partner-preview-copy"><span className="section-kicker">VISTA PREVIA</span><h2 id="partner-preview-title">{draft.publicName || 'Tu nombre'}</h2><strong>{draft.specialty || 'Tu profesión o especialidad'}</strong><h3>Quién soy</h3><RichBio value={draft.bio || 'Agrega una descripción para contarle a tus clientes quién eres y cómo trabajas.'}/><div className="partner-preview-contact">{draft.publicWhatsapp && draft.whatsapp && <span>WhatsApp</span>}{draft.publicEmail && draft.email && <span>Email</span>}{!hasContact && <small>Elige al menos un dato de contacto público.</small>}</div></div></div></div>}
  </div>
}
