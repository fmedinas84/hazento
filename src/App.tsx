...
function CreateForm({ type, labels, onDone }: { type: string; labels: typeof verticalLabels[Vertical]; onDone: () => void }) {
  ...
  const selectedAccount = store.accounts.find(account => account.name === accountName)
  ...
  const submit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (mode === 'account') {
      ...
      store.addPrestation({ accountId: selectedAccount.id, date: formatScheduledDate(String(values.date || '2026-08-17'), String(values.time || '09:00')), account: accountName, name: selectedService?.name || name, origin: String(values.origin || 'Directa'), status: String(values.status || 'Programada'), amount: String(values.amount || selectedService?.price || '$0'), payment: 'Pendiente', followUpNote: labels.supportsFollowUp ? String(values.followUpNote || '').trim() : undefined })
    } else if (mode === 'activity') {
      store.addActivity({ title: name, relation: `${accountName} · ${String(values.activityType || 'Tarea')}`, date: 'Hoy', type: String(values.activityType || 'Tarea'), status: 'Pendiente', accountId: selectedAccount?.id })
...
function AccountsPage({ labels, go, onCreate }: { labels: typeof verticalLabels[Vertical]; go: Navigate; onCreate: () => void }) {
  ...
  return <><PageHeader title={labels.accounts} description={`Gestiona relaciones, actividad, ${labels.engagements.toLowerCase()} y cobros de cada ${labels.account.toLowerCase()}.`} action={newAccountLabel(labels)} onAction={onCreate}/>
...
function AccountDetail({ labels, go, onCreate }: { labels: typeof verticalLabels[Vertical]; go: Navigate; onCreate: (type?: string) => void }) {
  ...
  const accountPrestations=store.prestations.filter(item=>item.accountId===a.id)
  const accountOpportunities=store.opportunities.filter(item=>item.account===a.name)
  const accountActivities=store.activities.filter(item=>item.accountId===a.id||item.relation.startsWith(a.name))
  ...
  const accountFollowUps=accountPrestations.filter(item=>item.followUpNote?.trim()).map(prestation=>({prestation,activity:store.activities.find(activity=>activity.source==='prestation_follow_up'&&activity.prestationId===prestation.id)})).sort((left,right)=>new Date(right.activity?.updatedAt||0).getTime()-new Date(left.activity?.updatedAt||0).getTime())
  ...
  const contextual: Array<{title:string;meta:string;value:string;description?:string;prestationId?:number}> = tab===labels.engagements ? accountEngagements.map(item=>({title:item.name,meta:`${labels.engagement} · ${item.status}`,value:item.amount})) : tab==='Oportunidades' ? accountOpportunities.map(item=>({title:item.title,meta:`${item.stage} · Cierre ${item.close}`,value:item.amount})) : tab==='Actividades' ? accountActivities.map(item=>({title:item.title,meta:`${item.type} · ${item.date}`,value:item.status,description:item.description,prestationId:item.prestationId})) : accountPayments.map(item=>({title:`Pago · ${item.method}`,meta:item.date,value:item.amount}))
  const contextualCreateType = tab===labels.engagements ? newEngagementLabel(labels) : tab==='Oportunidades' ? 'Nueva oportunidad' : tab==='Actividades' ? 'Nueva actividad' : tab==='Pagos' ? 'Registrar pago' : newPrestationLabel(labels)
  return <><button className="back-btn" onClick={()=>go('accounts')}><ChevronLeft size={17}/>Volver a {labels.accounts.toLowerCase()}</button>
    ...
    <div className="detail-tabs">{['Resumen',labels.engagements,'Oportunidades','Actividades','Pagos','Datos'].map(t=><button className={tab===t?'active':''} onClick={()=>setTab(t)} key={t}>{t}</button>)}</div>
    {tab==='Resumen'?<div className="account-layout"><main><div className="mini-metrics"><div><span>Ingreso histórico</span><b>{a.income}</b></div><div><span>Pendiente</span><b className="orange-text">{a.pending}</b></div><div><span>{labels.prestations}</span><b>{accountPrestations.length}</b></div><div><span>Oportunidades</span><b>{accountOpportunities.length}</b></div></div><section className="card timeline-card"><div className="card-heading"><h2>Historia con {a.name.split(' ')[0]}</h2><button className="text-btn" onClick={()=>onCreate('Nueva actividad')}>Agregar actividad <Plus size={15}/></button></div>{[...accountPrestations.map(item=>({date:item.date.split(' · ')[0],title:`${labels.prestation} ${item.status.toLowerCase()}`,meta:item.name,value:item.amount})),...accountPayments.map(item=>({date:item.date,title:'Pago recibido',meta:item.method,value:item.amount}))].slice(0,6).map((item,i)=><div className="timeline-item" key={`${item.title}-${i}`}><time>{item.date}</time><span className="timeline-dot"/><div><b>{item.title}</b><span>{item.meta}</span></div><strong>{item.value}</strong></div>)}</section>{labels.supportsFollowUp&&<section className="card followup-card"><div className="card-heading"><div><span className="section-kicker">Seguimientos</span><h2>Notas de atenciones</h2></div><span className="followup-count">{accountFollowUps.length}</span></div>{accountFollowUps.length?accountFollowUps.map(({prestation,activity})=><button className="followup-row" onClick={()=>go('prestations',{id:prestation.id})} key={prestation.id}><time>{activity?.date||prestation.date.split(' · ')[0]}</time><div><b>{prestation.name}</b><p>{prestation.followUpNote}</p></div><ChevronRight size={16}/></button>):<p className="followup-empty">Sin seguimientos registrados para este paciente.</p>}</section>}</main><aside><section className="card next-card"><span className="section-kicker">Próximo compromiso</span><div className="date-box"><b>24</b><span>AGO</span></div><h3>{accountPrestations[0]?.name || `Sin ${labels.prestations.toLowerCase()}`}</h3><p>{accountPrestations[0]?.date || 'Agenda una próxima fecha'}</p><button className="secondary-btn" onClick={()=>go('agenda',{account:a.id})}>Ver en agenda</button></section><section className="card info-card"><h3>Datos de contacto</h3>{editing?<form onSubmit={event=>{event.preventDefault();const values=Object.fromEntries(new FormData(event.currentTarget).entries());store.updateAccount(a.id,{email:String(values.email),phone:String(values.phone),rut:String(values.rut)});setEditing(false)}}><input name="email" defaultValue={a.email}/><input name="phone" defaultValue={a.phone}/><input name="rut" defaultValue={a.rut}/><button className="primary-btn">Guardar</button></form>:<><p><span>Email</span>{a.email||'—'}</p><p><span>Teléfono</span>{a.phone||'—'}</p><p><span>RUT</span>{a.rut||'—'}</p><button className="text-btn" onClick={()=>setEditing(true)}>Editar datos</button></>}</section></aside></div>:tab==='Datos'?<section className="card placeholder-section"><div className="card-heading"><div><span className="section-kicker">Información comercial</span><h2>Datos de {a.name}</h2></div><button className="secondary-btn" onClick={()=>setEditing(true)}>Editar</button></div><div className="detail-data-grid"><p><span>Tipo</span><b>{a.type}</b></p><p><span>Estado</span><StatusBadge>{a.status}</StatusBadge></p><p><span>Email</span><b>{a.email||'—'}</b></p><p><span>Teléfono</span><b>{a.phone||'—'}</b></p><p><span>RUT</span><b>{a.rut||'—'}</b></p><p><span>{labels.engagements}</span><b>{accountEngagements.length}</b></p></div></section>:<section className="card contextual-list"><div className="card-heading"><div><span className="section-kicker">{a.name}</span><h2>{tab}</h2></div><button className="secondary-btn" onClick={()=>onCreate(contextualCreateType)}><Plus size={15}/>Agregar</button></div>{contextual.length?contextual.map((item,i)=><div className="context-row" key={i}><div><b>{item.title}</b><span>{item.meta}</span>{item.description&&<p>{item.description}</p>}{item.prestationId&&<button className="text-btn" onClick={()=>go('prestations',{id:item.prestationId!})}>Abrir {labels.prestation.toLowerCase()}</button>}</div><strong>{item.value}</strong></div>):<EmptyState title={`Sin ${tab.toLowerCase()}`} body={`Aún no hay registros relacionados con ${a.name}.`} action="Agregar primero" onAction={()=>onCreate(contextualCreateType)}/>}</section>}
  </>
}
