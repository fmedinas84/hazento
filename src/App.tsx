...
function CreateForm({ type, labels, onDone, initialAccountId, initialEngagementId }: { type: string; labels: typeof verticalLabels[Vertical]; onDone: () => void; initialAccountId?: number; initialEngagementId?: number }) {
  ...
  return <form onSubmit={submit}>
    <div className="form-grid">
      ...
      {emailFirstMode && selectedAccount && <div className={cls('selected-person-context form-span', contextLocked && 'inherited-context')}><span><small>{contextLocked ? 'Contexto heredado' : labels.account}</small><b>{selectedAccount.name}</b><small>{selectedOrganization?.name || 'Independiente'}{selectedAccount.role ? ` · ${selectedAccount.role}` : ''}</small>{contextLocked && inheritedEngagement && <small>{labels.engagement}: {inheritedEngagement.name}</small>}</span>{contextLocked && <button type="button" className="text-btn" onClick={() => setContextLocked(false)}>Cambiar contexto</button>}</div>}
...
function Dashboard({ labels, go, onCreatePrestation }: { labels: typeof verticalLabels[Vertical]; go: Navigate; onCreatePrestation: () => void }) {
  ...
  return <>
    ...
    <div className="metrics-grid">
      <button className="dashboard-create-card" onClick={onCreatePrestation}><span className="dashboard-create-icon"><Plus size={20}/></span><span>{labels.createPrestation}</span><strong>Agendar ahora</strong><ChevronRight size={18}/></button>
