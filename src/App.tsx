...
import { Vertical, verticalLabels } from './data'
...
function CreateForm({ type, labels, onDone }: { type: string; labels: typeof verticalLabels[Vertical]; onDone: () => void }) {
  ...
  const submit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (mode === 'account') {
      ...
      store.addPrestation({ date: formatScheduledDate(String(values.date || '2026-08-17'), String(values.time || '09:00')), account: accountName, name: selectedService?.name || name, origin: String(values.origin || 'Directa'), status: 'Programada', amount: selectedService?.price || String(values.amount || '$0'), payment: 'Pendiente' })
...
function AccountDetail({ labels, go, onCreate }: { labels: typeof verticalLabels[Vertical]; go: Navigate; onCreate: (type?: string) => void }) {
  const store = useRepositories(); const [tab,setTab]=useState('Resumen'); const [editing,setEditing]=useState(false)
...
function RepositorySettingsPage({ vertical, setVertical, notify }: { vertical: Vertical; setVertical: (value: Vertical) => void; notify: (message: string) => void }) {
  ...
  const professions: Array<[Vertical, string, string, React.ElementType]> = [
    ['health','Psicólogo / Salud','Pacientes, tratamientos y atenciones',HeartPulse],
    ['creative','Diseñador / Creative','Clientes, proyectos y entregables',BriefcaseBusiness],
    ['creator','Influencer / Creator','Marcas, partnerships y contenidos',Sparkles],
    ['sessions','Profesor / Sesiones','Alumnos, programas y clases',CalendarDays],
  ]
  return <>
...
function App() {
  ...
  const createOptions: Array<[string, React.ElementType]> = vertical==='health'||vertical==='sessions'?[[newAccountLabel(labels),UserRound],[newPrestationLabel(labels),CalendarDays],['Nueva actividad',ListChecks],['Registrar pago',CreditCard]]:[[newAccountLabel(labels),UserRound],['Nueva oportunidad',Target],[`Nuevo ${labels.engagement.toLowerCase()}`,BriefcaseBusiness],[newPrestationLabel(labels),CalendarDays],['Registrar pago',CreditCard]]
