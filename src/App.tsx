...
import {
  Activity, BriefcaseBusiness, Building2, CalendarDays, Check, ChevronDown, ChevronLeft,
...
import { PartnerLandingSettings } from './PartnerLandingSettings'
import { PartnerSchedulingSettings } from './PartnerSchedulingSettings'
...
function CreateForm({ type, labels, onDone, initialAccountId, initialEngagementId }: { type: string; labels: typeof verticalLabels[Vertical]; onDone: () => void; initialAccountId?: string; initialEngagementId?: string }) {
  ...
  return <form onSubmit={submit}>
    <div className="form-grid">
      ...
      {emailFirstMode && selectedAccount && <div className="selected-person-context inherited-context form-span"><span><small>{labels.account}</small><b>{selectedAccount.name}</b><small>{selectedOrganization?.name || 'Independiente'}{selectedAccount.role ? ` · ${selectedAccount.role}` : ''}</small>{contextLocked && inheritedEngagement && <small>{labels.engagement}: {inheritedEngagement.name}</small>}</span>{contextLocked && <button type="button" className="text-btn" onClick={() => setContextLocked(false)}>Cambiar contexto</button>}</div>}
...
function OpportunityEditModal({ opportunityId, labels, onClose }: { opportunityId: string; labels: Labels; onClose: () => void }) {
  ...
  return <Modal title="Editar oportunidad" subtitle={`${personLabel}: ${opportunity.account}`} onClose={onClose} wide><form onSubmit={save}>
    <div className="form-grid">
      ...
      {account && <div className="selected-person-context inherited-context form-span"><span><small>{personLabel}</small><b>{account.name}</b><small>{organization?.name || 'Independiente'}{account.role ? ` · ${account.role}` : ''}</small></span></div>}
...
function RepositorySettingsPage({ vertical, setVertical, notify }: { vertical: Vertical; setVertical: (value: Vertical) => void; notify: (message: string) => void }) {
  ...
  return <>
    ...
    <div className="settings-layout"><aside className="settings-nav card">{['Perfil','Negocio','Landing pública','Agendamiento','Preferencias','Recordatorios','Integraciones','Facturación'].map(item => <button className={tab === item ? 'active' : ''} onClick={() => setTab(item)} key={item}>{item}</button>)}</aside><main className="card settings-card">
      ...
      {!['Facturación','Landing pública','Agendamiento','Integraciones','Recordatorios'].includes(tab) && <footer><button className="primary-btn" onClick={save}>Guardar cambios</button></footer>}
    </main></div>
  </>
}
...
function App() {
  ...
  const openCreate=(type?:string,accountId?:string,engagementId?:string)=>{if(store.repositoryStatus!=='ready')return;setCreateType(type||newPrestationLabel(labels));setCreateAccountId(accountId);setCreateEngagementId(engagementId);setCreateOpen(true);setCreateMenu(false)}
  ...
  return <div className={cls('app-shell',collapsed&&'sidebar-collapsed',mobileOpen&&'mobile-sidebar-open',loading&&'repository-loading')} aria-busy={loading}>
    ...
    {createOpen&&!loading&&!failed&&<Modal title={createType} subtitle="Completa los datos principales. Podrás agregar más detalle después." onClose={()=>setCreateOpen(false)} wide={createType==='Registrar pago'}><CreateForm type={createType} labels={labels} initialAccountId={createAccountId} initialEngagementId={createEngagementId} onDone={done}/></Modal>}
