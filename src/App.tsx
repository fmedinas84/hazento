...
function OpportunityStatusActions({ id, status }: { id: number; status: string }) {
  ...
  return <div className="opportunity-status-actions"><button className={status === 'Ganada' ? 'active won' : ''} onClick={() => updateOpportunity(id, { status: 'Ganada', last: 'Ganada · Ahora' })}><Check size={14}/>Ganada</button><button className={status === 'Perdida' ? 'active lost' : ''} onClick={() => updateOpportunity(id, { status: 'Perdida', last: 'Perdida · Ahora' })}><X size={14}/>Perdida</button></div>
}
