...
function RepositorySettingsPage({ vertical, setVertical, notify }: { vertical: Vertical; setVertical: (value: Vertical) => void; notify: (message: string) => void }) {
  ...
  return <>
    ...
    <div className="settings-layout"><aside className="settings-nav card">{['Perfil','Negocio','Landing pública','Disponibilidad','Preferencias','Recordatorios','Integraciones','Facturación'].map(item => <button className={tab === item ? 'active' : ''} onClick={() => setTab(item)} key={item}>{item}</button>)}</aside><main className="card settings-card">
      ...
      {!['Facturación','Landing pública','Disponibilidad','Integraciones','Recordatorios'].includes(tab) && <footer><button className="primary-btn" onClick={save}>Guardar cambios</button></footer>}
    </main></div>
  </>
}
