...
function CreateForm({ type, labels, onDone, initialAccountId, initialEngagementId }: { type: string; labels: typeof verticalLabels[Vertical]; onDone: () => void; initialAccountId?: number; initialEngagementId?: number }) {
  ...
  const submit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (mode === 'account') {
      ...
      store.addOpportunity({ accountId: selectedAccount.id, account: selectedAccount.name, title: name, amount: formatMoney(parseMoney(String(values.amount || '$0'))), close: formatShortDate(String(values.date || '2026-08-30')), contact: selectedAccount.name, last: 'Ahora', stage: String(values.stage || 'Nuevo'), status: 'Abierta' })
...
function OpportunityEditModal({ opportunityId, labels, onClose }: { opportunityId: number; labels: Labels; onClose: () => void }) {
  const store = useRepositories()
  const opportunity = store.opportunities.find(item => item.id === opportunityId)
  const [accountId, setAccountId] = useState<number | null>(opportunity?.accountId || null)
  ...
  const save = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!account) return
    const values = Object.fromEntries(new FormData(event.currentTarget).entries())
    store.updateOpportunity(opportunity.id, { accountId: account.id, account: account.name, contact: account.name, title: String(values.name).trim(), amount: formatMoney(parseMoney(String(values.amount))), close: formatShortDate(String(values.date)), stage: String(values.stage) })
    onClose()
  }
  return <Modal title="Editar oportunidad" subtitle={`${personLabel}: ${opportunity.account}`} onClose={onClose} wide><form onSubmit={save}>
    <div className="form-grid">
      ...
      <label><span>Etapa</span><select name="stage" defaultValue={opportunity.stage}>{['Nuevo','Contactado','Propuesta','Negociación'].map(item => <option key={item}>{item}</option>)}</select></label>
...
function OpportunityStatusActions({ id, status }: { id: number; status: string }) {
  const { updateOpportunity } = useRepositories()
  return <div className="opportunity-status-actions"><button type="button" aria-pressed={status === 'Ganada'} className={status === 'Ganada' ? 'active won' : ''} onClick={() => updateOpportunity(id, { status: 'Ganada', last: 'Ganada · Ahora' })}><Check size={14}/>Ganada</button><button type="button" aria-pressed={status === 'Perdida'} className={status === 'Perdida' ? 'active lost' : ''} onClick={() => updateOpportunity(id, { status: 'Perdida', last: 'Perdida · Ahora' })}><X size={14}/>Perdida</button></div>
}
...
function OpportunitiesPage({ labels, go, onCreate }: { labels: Labels; go:Navigate; onCreate:()=>void }) {
  ...
  const pipelineAmount = open.reduce((sum, item) => sum + parseMoney(item.amount), 0)
  return <><PageHeader className="mobile-labeled-action" title="Oportunidades" description={`Haz seguimiento a cada posibilidad hasta convertirla en un ${labels.engagement.toLowerCase()} activo.`} action="Crear oportunidad" onAction={onCreate}/>
    <div className="pipeline-summary card"><div><span>Pipeline abierto</span><b>{formatMoney(pipelineAmount)}</b></div><div><span>Oportunidades abiertas</span><b>{open.length}</b></div><div><span>Ganadas</span><b>{opportunities.filter(item => item.status === 'Ganada').length}</b></div><div><span>Requieren seguimiento</span><b>{open.filter(item=>item.last.includes('7 días')).length}</b></div></div>
    ...
    {view === 'cards' ? <div className="kanban">{stages.map(stage=><section className="kanban-column" onDragOver={event=>event.preventDefault()} onDrop={event=>updateOpportunity(Number(event.dataTransfer.getData('opportunityId')),{stage})} key={stage}><header><span><i className={`stage-dot ${stage.toLowerCase()}`}/>{stage}</span><b>{opportunities.filter(o=>o.stage===stage).length}</b></header>{opportunities.filter(o=>o.stage===stage).map(o=><article draggable onDragStart={event=>event.dataTransfer.setData('opportunityId',String(o.id))} className={cls('opportunity-card card',o.last.includes('7 días')&&'needs-attention',o.status === 'Ganada'&&'is-won',o.status === 'Perdida'&&'is-lost')} onClick={()=>go('opportunity',{id:o.id})} key={o.id}><div className="opportunity-card-head"><span>{o.account}</span><button className="icon-row-action" aria-label={`Editar ${o.title}`} onClick={event => { event.stopPropagation(); setEditing(o.id) }}><Pencil size={14}/></button></div><h3>{o.title}</h3><div className="opportunity-card-meta"><strong>{o.amount}</strong><span><CalendarDays size={13}/>{o.close}</span></div><div className="opportunity-card-controls" onClick={event=>event.stopPropagation()}><select aria-label={`Etapa de ${o.title}`} value={o.stage} onChange={event=>updateOpportunity(o.id,{stage:event.target.value})}>{stages.map(item=><option key={item}>{item}</option>)}</select><OpportunityStatusActions id={o.id} status={o.status || 'Abierta'}/></div></article>)}<button className="add-card" onClick={onCreate}><Plus size={15}/>Agregar oportunidad</button></section>)}</div> : <div className="table-card card opportunity-list"><table><thead><tr><th>{opportunityPersonLabel(labels)}</th><th>Oportunidad</th><th>Etapa</th><th>Estado</th><th>Cierre</th><th>Monto</th><th>Acciones</th></tr></thead><tbody>{opportunities.map(item => <tr key={item.id} onClick={() => go('opportunity', { id: item.id })}><td><b>{item.account}</b></td><td>{item.title}</td><td><select aria-label={`Etapa de ${item.title}`} value={item.stage} onClick={event => event.stopPropagation()} onChange={event => updateOpportunity(item.id, { stage: event.target.value })}>{stages.map(stage => <option key={stage}>{stage}</option>)}</select></td><td><StatusBadge>{item.status || 'Abierta'}</StatusBadge></td><td>{item.close}</td><td className="number">{item.amount}</td><td><button className="icon-row-action" aria-label={`Editar ${item.title}`} onClick={event => { event.stopPropagation(); setEditing(item.id) }}><Pencil size={14}/></button></td></tr>)}</tbody></table></div>}
...
function OpportunityDetail({ labels, go, onCreate }: { labels: typeof verticalLabels[Vertical]; go:Navigate; onCreate:(type?:string)=>void }) {
  ...
  const organization = store.organizations.find(item => item.id === account?.organizationId)
  const win=(create:boolean)=>{store.updateOpportunity(opportunity.id,{status:'Ganada',last:'Ganada · Ahora'});if(create){store.addEngagement({accountId:opportunity.accountId,opportunityId:opportunity.id,name:opportunity.title,account:opportunity.account,type:labels.engagement,progress:0,detail:`0 ${labels.prestations.toLowerCase()}`,amount:opportunity.amount,status:'Activo'});go('work')}else{setWon(false)}}
  return <><button className="back-btn" onClick={()=>go('opportunities')}><ChevronLeft size={17}/>Volver a oportunidades</button><section className="detail-hero card"><div><span className="section-kicker">Oportunidad · {opportunity.stage}</span><h1>{opportunity.title}</h1><p>{opportunity.account}{organization ? ` · ${organization.name}` : ''}</p></div><div className="opportunity-value"><b>{opportunity.amount}</b><StatusBadge>{status}</StatusBadge><button className="secondary-btn" onClick={() => setEditing(true)}><Pencil size={15}/>Editar</button></div></section><div className="opportunity-layout"><main><section className="card timeline-card"><div className="card-heading"><h2>Actividad de la oportunidad</h2><button className="secondary-btn" onClick={()=>onCreate('Nueva actividad')}><Plus size={16}/>Agregar actividad</button></div>{[['17 Ago','Seguimiento pendiente','Recordatorio para contactar'],['12 Ago','Propuesta enviada',opportunity.title],['08 Ago','Reunión realizada',`${opportunity.account} · 45 minutos`],['04 Ago','Oportunidad creada','Origen: recomendación']].map((x,i)=><div className="timeline-item" key={i}><time>{x[0]}</time><span className="timeline-dot"/><div><b>{x[1]}</b><span>{x[2]}</span></div></div>)}</section></main><aside><section className="card detail-fields"><h3>Detalles</h3><p><span>{personLabel}</span><b>{opportunity.account}</b></p>{organization && <p><span>Organización</span><b>{organization.name}</b></p>}<p><span>Monto</span><b>{opportunity.amount}</b></p><p><span>Etapa</span><select value={opportunity.stage} onChange={event=>store.updateOpportunity(opportunity.id,{stage:event.target.value})}>{['Nuevo','Contactado','Propuesta','Negociación'].map(item=><option key={item}>{item}</option>)}</select></p><OpportunityStatusActions id={opportunity.id} status={status}/><p><span>Cierre estimado</span><b>{opportunity.close}</b></p></section>{status !== 'Ganada' && <button className="win-btn" onClick={()=>setWon(true)}><Check size={18}/>Ganada</button>}{status !== 'Perdida' && <button className="lost-btn" onClick={()=>store.updateOpportunity(opportunity.id,{status:'Perdida',last:'Perdida · Ahora'})}>Perdida</button>}</aside></div>{editing&&<OpportunityEditModal opportunityId={opportunity.id} labels={labels} onClose={()=>setEditing(false)}/>} {won&&<Modal title="¡Oportunidad ganada!" subtitle="Elige explícitamente qué debe ocurrir con esta venta." onClose={()=>setWon(false)}><div className="choice-list"><button onClick={()=>win(true)}><span className="choice-icon"><BriefcaseBusiness size={19}/></span><div><b>Crear {labels.engagement.toLowerCase()}</b><span>Organiza el {labels.engagement.toLowerCase()} y sus próximas {labels.prestations.toLowerCase()}.</span></div><ChevronRight size={17}/></button><button onClick={()=>win(false)}><span className="choice-icon"><Check size={19}/></span><div><b>Solo cerrar oportunidad</b><span>No crear {labels.engagement.toLowerCase()} asociado por ahora.</span></div><ChevronRight size={17}/></button></div></Modal>}</>
}
...
function PaymentEditModal({ paymentId, labels, onClose }: { paymentId: number; labels: Labels; onClose: () => void }) {
  const repositories = useRepositories()
  const payment = repositories.payments.find(item => item.id === paymentId)
  const currentAllocations = repositories.paymentAllocations.filter(item => item.paymentId === paymentId)
  const [selected, setSelected] = useState<number[]>(() => currentAllocations.map(item => item.prestationId))
  if (!payment) return null
  const candidates = repositories.prestations.filter(prestation => prestation.accountId === payment.accountId && (repositories.prestationRepository.balanceFor(prestation.id) > 0 || currentAllocations.some(item => item.prestationId === prestation.id)))
  const save = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    const values = Object.fromEntries(new FormData(event.currentTarget).entries())
    const amount = parseMoney(String(values.amount))
    let remaining = amount
    const allocations = selected.map(prestationId => {
      const current = currentAllocations.find(item => item.prestationId === prestationId)?.amount || 0
      const restorable = payment.status === 'Pagado' ? current : 0
      const capacity = repositories.prestationRepository.balanceFor(prestationId) + restorable
      const allocated = Math.min(remaining, capacity)
      remaining -= allocated
      return { prestationId, amount: allocated }
    }).filter(item => item.amount > 0)
    repositories.paymentRepository.update(payment.id, { amount: formatMoney(amount), date: formatShortDate(String(values.date)), method: String(values.method), status: String(values.status), allocations: allocations.length ? `${allocations.length} ${labels.prestations.toLowerCase()}` : 'Sin asignar' }, allocations)
    onClose()
  }
  return <Modal title="Editar pago" subtitle={`${payment.account} · ${payment.amount}`} onClose={onClose} wide><form onSubmit={save}>
    <div className="form-grid"><label><span>Monto *</span><input name="amount" required autoFocus defaultValue={payment.amount}/></label><label><span>Fecha</span><input name="date" type="date" defaultValue={shortDateToInput(payment.date)}/></label><label><span>Método</span><select name="method" defaultValue={payment.method}>{['Transferencia','Efectivo','Tarjeta'].map(item => <option key={item}>{item}</option>)}</select></label><label><span>Estado</span><select name="status" defaultValue={payment.status}><option>Pagado</option><option>Pendiente</option><option>Cancelado</option></select></label></div>
    <div className="allocation-box"><div><strong>Asignar a {labels.prestations.toLowerCase()}</strong><p>Al guardar se reemplazan las asignaciones actuales; nunca se duplica el pago.</p></div>{candidates.length ? candidates.map(prestation => <label className="allocation" key={prestation.id}><input type="checkbox" checked={selected.includes(prestation.id)} onChange={() => setSelected(current => current.includes(prestation.id) ? current.filter(id => id !== prestation.id) : [...current, prestation.id])}/><span>{prestation.date}<b>{prestation.name}</b></span><strong>{formatMoney(repositories.prestationRepository.balanceFor(prestation.id) + (payment.status === 'Pagado' ? currentAllocations.find(item => item.prestationId === prestation.id)?.amount || 0 : 0))}</strong></label>) : <p className="form-empty">No hay {labels.prestations.toLowerCase()} disponibles para asignar.</p>}</div>
    <footer className="modal-actions"><button type="button" className="ghost-btn" onClick={onClose}>Cancelar</button><button className="primary-btn">Guardar cambios</button></footer>
  </form></Modal>
}
...
function RepositoryPaymentsPage({ labels, onCreate }: { labels: Labels; onCreate: () => void }) {
  ...
  return <>
    ...
    {editing && <PaymentEditModal paymentId={editing} labels={labels} onClose={() => setEditing(null)}/>}</>
}
