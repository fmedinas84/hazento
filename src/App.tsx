...
function App() {
  ...
  if (store.repositoryStatus === 'loading') return <main className="repository-state" aria-live="polite"><div className="card"><h1>Cargando Hazento...</h1><p>Estamos preparando tus datos.</p></div></main>
  if (store.repositoryStatus === 'error') return <main className="repository-state" role="alert"><div className="card"><h1>No pudimos cargar tus datos</h1><p>{store.repositoryError}</p><button className="primary-btn" onClick={store.retryRepository}>Reintentar</button></div></main>
