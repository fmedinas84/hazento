...
function CreateForm({ type, labels, onDone, initialAccountId }: { type: string; labels: typeof verticalLabels[Vertical]; onDone: () => void; initialAccountId?: number }) {
  ...
  return <form onSubmit={submit}>
    <div className="form-grid">
      {emailFirstMode && <AccountEmailSelector accounts={store.accounts} labels={labels} selectedAccountId={accountId} onSelect={account => { setAccountId(account?.id || null); setAccountError('') }} onCreate={store.accountRepository.create} organizations={store.organizations} onCreateOrganization={store.organizationRepository.create}/>}
      {accountError && <p className="form-error form-span">{accountError}</p>}
      {emailFirstMode && selectedAccount && <div className="selected-person-context form-span"><span><b>{selectedAccount.name}</b><small>{store.organizations.find(organization => organization.id === selectedAccount.organizationId)?.name || 'Independiente'}{selectedAccount.role ? ` · ${selectedAccount.role}` : ''}</small></span></div>}
