...
function CreateForm({ type, labels, onDone, initialAccountId }: { type: string; labels: typeof verticalLabels[Vertical]; onDone: () => void; initialAccountId?: number }) {
  ...
  const submit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (mode === 'account') {
      ...
      store.accountRepository.create({ name, type: String(values.accountType || 'Persona'), status: 'Prospecto', last: 'Ahora', next: '—', income: '$0', pending: '$0', email, phone: String(values.phone || ''), rut: String(values.rut || '') })
      ...
      store.addPrestation({ accountId: selectedAccount.id, engagementId, serviceId: prestationServiceId, date: formatScheduledDate(String(values.date || '2026-08-17'), String(values.time || '09:00')), account: selectedAccount.name, name: prestationName, description: String(values.description || '').trim(), origin: engagementId ? labels.engagement : 'Directa', status: String(values.status || 'Programada'), amount: String(values.amount || selectedService?.price || '$0'), payment: 'Pendiente', followUpNote: labels.supportsFollowUp ? String(values.followUpNote || '').trim() : undefined })
  ...
  return <form onSubmit={submit}>
    <div className="form-grid">
      {emailFirstMode && <AccountEmailSelector accounts={store.accounts} labels={labels} selectedAccountId={accountId} onSelect={account => { setAccountId(account?.id || null); setAccountError('') }} onCreate={store.accountRepository.create}/>}
...
function AccountDetail({ labels, go, onCreate }: { labels: typeof verticalLabels[Vertical]; go: Navigate; onCreate: (type?: string) => void }) {
  ...
  const accountOpportunities=store.opportunities.filter(item=>item.accountId===a.id||(!item.accountId&&item.account===a.name))
  ...
  const accountEngagements=store.engagements.filter(item=>item.accountId===a.id||(!item.accountId&&item.account===a.name))
