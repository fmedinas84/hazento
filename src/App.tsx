...
import {
  ...
  HeartPulse, LayoutDashboard, Link2, ListChecks, Menu, MoreHorizontal, Pencil, Plus, Search, Settings,
...
function CreateForm({ type, labels, onDone, initialAccountId, initialEngagementId }: { type: string; labels: typeof verticalLabels[Vertical]; onDone: () => void; initialAccountId?: number; initialEngagementId?: number }) {
  ...
  const [serviceId, setServiceId] = useState<number>(scenarioServices[0]?.id || 0)
...
function Dashboard({ labels, go, onCreatePrestation }: { labels: typeof verticalLabels[Vertical]; go: Navigate; onCreatePrestation: () => void }) {
  ...
  return <>
    <PageHeader eyebrow={new Intl.DateTimeFormat('es-CL', { weekday: 'long', day: 'numeric', month: 'long' }).format(now)} title="Buenos días, Francisca" description="Aquí tienes lo más importante para mover tu negocio hoy." />
    ...
    <div className="dashboard-grid">
      ...
      <section className="card recent-card"><div className="card-heading"><div><span className="section-kicker">Últimos movimientos</span><h2>Actividad reciente</h2></div><MoreHorizontal size={19} /></div>
        {[['MP','María Pérez','Pago registrado · $35.000','Hace 20 min','green'],['PG','Pedro González','Oportunidad pasó a Propuesta','Hace 2 h','violet'],['CD','Carolina Díaz',labels.completedActivity,'Hace 3 h','orange'],['JS','Juan Soto','Nueva actividad programada','Ayer','blue']].map((r,i)=><div className="recent-row" key={i}><span className={cls('avatar-sm',r[4])}>{r[0]}</span><div><b>{r[1]}</b><span>{r[2]}</span></div><time>{r[3]}</time></div>)}
      </section>
    </div>
  </>
}
...
function OpportunityStatusActions({ id, status, onWon }: { id: number; status: string; onWon?: () => void }) {
...
function OpportunitiesPage({ labels, go, onCreate, onCreatePrestation }: { labels: Labels; go:Navigate; onCreate:()=>void; onCreatePrestation:(accountId:number)=>void }) {
  ...
  const [wonOpportunity, setWonOpportunity] = useState<number | null>(null)
...
function RepositorySettingsPage({ vertical, setVertical, notify }: { vertical: Vertical; setVertical: (value: Vertical) => void; notify: (message: string) => void }) {
  ...
  const save = () => {
    if (tab === 'Perfil') {
      ...
      repositories.updateProfile({ ...profile, firstName: profile.firstName.trim(), lastName: profile.lastName.trim(), email })
    }
    if (tab === 'Negocio') repositories.updateWorkspace({ ...workspace, name: workspace.name.trim() || repositories.workspace.name })
  ...
  return <>
    ...
    <div className="settings-layout"><aside className="settings-nav card">{['Perfil','Negocio','Preferencias','Integraciones','Facturación'].map(item => <button className={tab === item ? 'active' : ''} onClick={() => setTab(item)} key={item}>{item}</button>)}</aside><main className="card settings-card">
      {tab === 'Facturación' ? <BillingSettings/> : tab === 'Integraciones' ? <IntegrationsSettings notify={notify}/> : tab === 'Negocio' ? <><div><span className="section-kicker">Workspace demo</span><h2>Datos del negocio</h2><p>La profesión cambia labels, navegación y acciones. Los objetos y datos internos permanecen intactos.</p></div><div className="form-grid"><label><span>Nombre del negocio</span><input value={workspace.name} onChange={event => setWorkspace(current => ({ ...current, name: event.target.value }))}/></label><label><span>País</span><select value={workspace.country} onChange={event => setWorkspace(current => ({ ...current, country: event.target.value }))}><option>Chile</option></select></label><label><span>Moneda</span><select value={workspace.currency} onChange={event => setWorkspace(current => ({ ...current, currency: event.target.value }))}><option value="CLP">Peso chileno (CLP)</option></select></label><label><span>Zona horaria</span><select value={workspace.timezone} onChange={event => setWorkspace(current => ({ ...current, timezone: event.target.value }))}><option>America/Santiago</option></select></label></div><div className="vertical-setting"><h3>Tipo de profesional</h3><p>Selector temporal para validar la verticalización durante esta etapa.</p><div>{verticalOptions.map(option => { const Icon = verticalIcons[option.value]; return <button className={vertical === option.value ? 'active' : ''} onClick={() => setVertical(option.value)} key={option.value}><span><Icon/></span><div><b>{option.profession}</b><small>{option.professionDescription}</small></div>{vertical === option.value && <Check size={18}/>}</button> })}</div></div></> : <><div><span className="section-kicker">{tab}</span><h2>{tab === 'Perfil' ? 'Tu información' : 'Preferencias de uso'}</h2><p>{tab === 'Perfil' ? 'Datos locales visibles en este workspace demo.' : 'Ajustes de la experiencia de producto.'}</p></div>{tab === 'Perfil' && <div className="form-grid"><label><span>Nombre</span><input value={profile.firstName} onChange={event => setProfile(current => ({ ...current, firstName: event.target.value }))}/></label><label><span>Apellido</span><input value={profile.lastName} onChange={event => setProfile(current => ({ ...current, lastName: event.target.value }))}/></label><label className="form-span"><span>Email</span><input type="email" autoComplete="email" value={profile.email} onChange={event => setProfile(current => ({ ...current, email: event.target.value }))}/></label><label><span>Teléfono</span><input value={profile.phone} onChange={event => setProfile(current => ({ ...current, phone: event.target.value }))}/></label></div>}{tab === 'Preferencias' && <div className="setting-options"><label><input type="checkbox" defaultChecked/> Mostrar pendientes prioritarios al iniciar</label><label><input type="checkbox" defaultChecked/> Confirmar antes de archivar</label><button className="secondary-btn" onClick={() => { if (window.confirm('¿Restaurar todos los datos de demostración?')) { repositories.resetDemo(); notify('Datos demo restaurados') } }}>Restaurar datos demo</button></div>}</>}
      {!['Facturación','Integraciones'].includes(tab) && <footer><button className="primary-btn" onClick={save}>Guardar cambios</button></footer>}
    </main></div>
  </>
}
