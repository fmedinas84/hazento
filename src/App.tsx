...
function CreateForm({ type, labels, onDone, initialAccountId }: { type: string; labels: typeof verticalLabels[Vertical]; onDone: () => void; initialAccountId?: number }) {
  ...
  const submit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (mode === 'account') {
      ...
      if (!selectedAccount) { setAccountError(`Selecciona ${labels.account.toLowerCase()} por email.`); return }
      store.addEngagement({ accountId: selectedAccount.id, opportunityId: Number(values.opportunityId) || undefined, name, account: selectedAccount.name, type: labels.engagement, progress: 0, detail: `0 ${labels.prestations.toLowerCase()}`, amount: String(values.amount || '$0'), status: 'Activo' })
