...
export function BillingSettings() {
  ...
  return <div className="billing-settings">
    ...
    <section className="billing-current card" aria-labelledby="billing-current-title">
      <div className="billing-section-heading"><span className="billing-section-icon"><StatusIcon size={18} className={subscription.status === 'pending' ? 'spin' : ''}/></span><div><span>Plan actual</span><h3 id="billing-current-title">{plusSelected ? 'PLUS' : 'FREE'}</h3><p>{plusSelected ? '$4.990 / mes' : '$0 / mes'}</p></div></div>
    ...
    <div className="billing-plans" aria-label="Planes disponibles">
      <article className={`billing-plan ${!plusSelected ? 'selected' : ''}`}><div><span>FREE</span><strong>$0</strong><small>CLP / mes</small></div><p>Organiza tu operación con las funciones esenciales de Hazento.</p><button className="secondary-btn" type="button" disabled={!plusSelected} onClick={() => plusSelected && updateSubscription('cancel')}>{!plusSelected ? 'Plan actual' : 'Volver a Free'}</button></article>
      <article className={`billing-plan billing-plan-plus ${plusSelected ? 'selected' : ''}`}><div><span>PLUS</span><strong>$4.990</strong><small>CLP / mes</small></div><p>Cobro mensual recurrente, administrado de forma segura por Mercado Pago.</p>{!plusSelected ? <button className="primary-btn" type="button" disabled={!publicKey || loading} onClick={() => { setError(''); setShowCheckout(true) }}>Cambiar a Plus</button> : <span className="billing-plan-active"><CheckCircle2 size={15}/> Plan seleccionado</span>}</article>
    </div>
    ...
    {showCheckout && !plusSelected && <section className="billing-section billing-payment" aria-labelledby="billing-payment-title">
      <div className="billing-section-heading"><span className="billing-section-icon"><CreditCard size={18}/></span><div><h3 id="billing-payment-title">Autoriza Hazento Plus</h3><p>Hazento Plus · $4.990 al mes · Cobro mensual recurrente</p></div></div>
