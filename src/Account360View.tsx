...
export function Account360View({ labels, go, onCreate }: { labels: Labels; go: Navigate; onCreate: CreateAction }) {
  ...
  return <div className="account360">
    ...
    {tab === 'Prestaciones' && <div className="account360-stack">
      ...
      <section className="card account360-section"><div className="card-heading"><div><span className="section-kicker">Unidades de trabajo</span><h2>{labels.prestations}</h2></div><button className="secondary-btn" onClick={() => create(labels.createPrestation)}><Plus size={15}/>{labels.createPrestation}</button></div>{prestations.length ? <div className="account360-table"><table><thead><tr><th>Fecha</th><th>{labels.prestation}</th><th>{labels.engagement}</th>{!labels.supportsFollowUp&&<th>Estado</th>}<th>Monto</th><th>Pago</th><th/></tr></thead><tbody>{prestations.map(record => { const engagement = engagements.find(item => item.id === record.engagementId); return <tr key={record.id} onClick={() => go('prestations', { id: record.id })}><td>{record.date}</td><td><b>{record.name}</b></td><td>{engagement?.name || 'Directa'}</td>{!labels.supportsFollowUp&&<td><Badge>{record.status}</Badge></td>}<td>{record.amount}</td><td><Badge>{repository.paymentRequestRepository.collectionStatusForPrestation(record.id)}</Badge></td><td><ChevronRight size={15}/></td></tr> })}</tbody></table></div> : <BlankState title={`Todavía no hay ${labels.prestations.toLowerCase()}`} body={`Registra la primera ${labels.prestation.toLowerCase()} para comenzar el historial.`} action={labels.createPrestation} onAction={() => create(labels.createPrestation)}/>}</section>
    </div>}
