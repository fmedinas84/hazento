...
function BlankState({ title, body, action, onAction }: { title: string; body: string; action?: string; onAction?: () => void }) {
  return <div className="account360-empty"><span><Plus size={18}/></span><h3>{title}</h3><p>{body}</p>{action && onAction && <button className="secondary-btn" onClick={onAction}>{action}</button>}</div>
}
  ...
  return <div className="account360">
    ...
    {tab === 'Resumen' && <div className="account360-layout">
      ...
      <aside>
        ...
        {futurePrestations.length > 0 && <section className="card account360-upcoming"><div className="card-heading"><h3>{['Entregable', 'Contenido'].includes(labels.prestation) ? 'Próximos' : 'Próximas'} {labels.prestations.toLowerCase()}</h3></div>{futurePrestations.slice(0, 3).map(record => <button key={record.id} onClick={() => go('prestations', { id: record.id })}><CalendarDays size={16}/><span><b>{record.name}</b><small>{record.date}</small></span><ChevronRight size={14}/></button>)}</section>}
      </aside>
    </div>}
    ...
    {tab === 'Prestaciones' && <div className="account360-stack">
      ...
      <section className="card account360-section"><div className="card-heading"><div><span className="section-kicker">Unidades de trabajo</span><h2>{labels.prestations}</h2></div><button className="secondary-btn" onClick={() => create(labels.createPrestation)}><Plus size={15}/>{labels.createPrestation}</button></div>{prestations.length ? <div className="account360-table"><table><thead><tr><th>Fecha</th><th>{labels.prestation}</th><th>{labels.engagement}</th>{!labels.supportsFollowUp&&<th>Estado</th>}<th>Monto</th><th>Pago</th><th/></tr></thead><tbody>{prestations.map(record => { const engagement = engagements.find(item => item.id === record.engagementId); return <tr key={record.id}><td>{record.date}</td><td><button className="table-link" onClick={() => go('prestations', { id: record.id })}>{record.name}</button></td><td>{engagement?.name || 'Directa'}</td>{!labels.supportsFollowUp&&<td><Badge>{record.status}</Badge></td>}<td>{record.amount}</td><td><Badge>{repository.paymentRequestRepository.collectionStatusForPrestation(record.id)}</Badge></td><td><button className="icon-row-action" aria-label={`Abrir ${record.name}`} onClick={() => go('prestations', { id: record.id })}><ChevronRight size={15}/></button></td></tr> })}</tbody></table></div> : <BlankState title={`Todavía no hay ${labels.prestations.toLowerCase()}`} body={`Registra la primera ${labels.prestation.toLowerCase()} para comenzar el historial.`} action={labels.createPrestation} onAction={() => create(labels.createPrestation)}/>}</section>
    </div>}
