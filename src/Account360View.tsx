...
import {
  Activity, Archive, BriefcaseBusiness, CalendarDays, Check, ChevronLeft, ChevronRight,
  CircleDollarSign, Clock3, CreditCard, Ellipsis, FileCheck2, Mail, MapPin, Pencil,
  Phone, Plus, Target, UserRound, UsersRound,
} from 'lucide-react'
...
export function Account360View({ labels, go, onCreate }: { labels: Labels; go: Navigate; onCreate: CreateAction }) {
  ...
  const contacts = repository.contacts.filter(record => record.accountId === accountId)
  ...
  const saveAccount = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    repository.accountRepository.update(account.id, {
      ...
      type: String(value.type), status: String(value.status), rut: String(value.rut || ''),
      legalName: String(value.legalName || ''), email: String(value.email || '').trim().toLowerCase(),
  ...
  const addContact = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    const value = Object.fromEntries(new FormData(event.currentTarget).entries())
    repository.contactRepository.create({ accountId: account.id, firstName: String(value.firstName), lastName: String(value.lastName), jobTitle: String(value.jobTitle || ''), email: String(value.email || ''), phone: String(value.phone || ''), isPrimary: contacts.length === 0 })
  ...
  return <div className="account360">
    ...
    <section className="account360-hero card">
      <div className="account360-identity"><span className="avatar-xl" style={{ background: account.color }}>{account.initials}</span><div><span className="section-kicker">{labels.account} {account.status.toLowerCase()}</span><h1>{account.name}</h1><div className="account360-contact"><span>{account.rut || 'Sin RUT'}</span><span><Mail size={13}/>{account.email || 'Sin email'}</span><span><Phone size={13}/>{account.phone || 'Sin teléfono'}</span></div></div></div>
    ...
    {tab === 'Resumen' && <div className="account360-layout">
      <main>
        ...
        <section className="card account360-active"><div className="card-heading"><div><span className="section-kicker">En curso</span><h2>{labels.engagements} activos</h2></div><button className="text-btn" onClick={() => setTab('Prestaciones')}>Ver todo <ChevronRight size={15}/></button></div>
          {activeEngagements.length ? activeEngagements.map(record => <button key={record.id} onClick={() => go('engagement', { id: record.id })}><span><b>{record.name}</b><small>{record.detail}</small></span><span className="progress-track"><i style={{ width: `${record.progress}%` }}/></span><strong>{record.progress}%</strong><ChevronRight size={15}/></button>) : <BlankState title={`Sin ${labels.engagements.toLowerCase()} activos`} body={`Agrupa aquí las próximas ${labels.prestations.toLowerCase()} acordadas.`} action={labels.createEngagement} onAction={() => create(labels.createEngagement)}/>}</section>
      </main>
      <aside>
    ...
    {tab === 'Prestaciones' && <div className="account360-stack">
      <section className="card account360-section"><div className="card-heading"><div><span className="section-kicker">Relación activa</span><h2>{labels.engagements}</h2></div><button className="secondary-btn" onClick={() => create(labels.createEngagement)}><Plus size={15}/>{labels.createEngagement}</button></div>{engagements.length ? engagements.map(record => <button className="account360-record" key={record.id} onClick={() => go('engagement', { id: record.id })}><BriefcaseBusiness size={17}/><span><b>{record.name}</b><small>{record.detail} · {record.status}</small></span><strong>{record.amount}</strong><ChevronRight size={15}/></button>) : <BlankState title={`No hay ${labels.engagements.toLowerCase()}`} body={`Crea un ${labels.engagement.toLowerCase()} para agrupar el trabajo acordado.`} action={labels.createEngagement} onAction={() => create(labels.createEngagement)}/>}</section>
    ...
    {tab === 'Datos' && <div className="account360-layout account360-data"><main><section className="card account360-section"><div className="card-heading"><div><span className="section-kicker">Información comercial</span><h2>Datos de {account.name}</h2></div>{!editing && <button className="secondary-btn" onClick={() => setEditing(true)}><Pencil size={15}/>Editar</button>}</div>{editing ? <form className="account360-form" onSubmit={saveAccount}><label><span>Nombre *</span><input name="name" required defaultValue={account.name}/></label><label><span>Tipo</span><select name="type" defaultValue={account.type}><option>Persona</option><option>Empresa</option><option>Marca</option></select></label><label><span>Estado</span><select name="status" defaultValue={account.status}><option>Prospecto</option><option>Activo</option><option>Inactivo</option></select></label><label><span>RUT</span><input name="rut" defaultValue={account.rut}/></label>{(account.type === 'Empresa' || account.type === 'Marca' || account.legalName) && <label><span>Razón social</span><input name="legalName" defaultValue={account.legalName}/></label>}<label><span>Email</span><input name="email" type="email" defaultValue={account.email}/></label><label><span>Teléfono</span><input name="phone" defaultValue={account.phone}/></label><label><span>Dirección</span><input name="address" defaultValue={account.address}/></label><label><span>Ciudad</span><input name="city" defaultValue={account.city}/></label><label><span>Comuna</span><input name="commune" defaultValue={account.commune}/></label><label className="wide"><span>Notas</span><textarea name="notes" rows={4} defaultValue={account.notes}/></label><footer><button type="button" className="ghost-btn" onClick={() => setEditing(false)}>Cancelar</button><button className="primary-btn">Guardar cambios</button></footer></form> : <div className="account360-data-grid"><p><span>Tipo</span><b>{account.type}</b></p><p><span>Estado</span><Badge>{account.status}</Badge></p><p><span>RUT</span><b>{account.rut || '—'}</b></p><p><span>Email</span><b>{account.email || '—'}</b></p><p><span>Teléfono</span><b>{account.phone || '—'}</b></p><p><span>Dirección</span><b>{[account.address, account.commune, account.city].filter(Boolean).join(', ') || '—'}</b></p>{account.legalName && <p><span>Razón social</span><b>{account.legalName}</b></p>}{account.notes && <p className="wide"><span>Notas</span><b>{account.notes}</b></p>}</div>}</section></main><aside><section className="card account360-section"><div className="card-heading"><div><span className="section-kicker">Personas relacionadas</span><h2>Contactos</h2></div>{(account.type === 'Empresa' || account.type === 'Marca' || contacts.length > 0) && <button className="text-btn" onClick={() => setAddingContact(true)}><Plus size={15}/>Agregar</button>}</div>{account.type === 'Persona' && contacts.length === 0 ? <p className="account360-muted">Los contactos se muestran para empresas o marcas.</p> : contacts.length ? contacts.map(contact => <div className="account360-contact-row" key={contact.id}><span><UserRound size={16}/></span><div><b>{contact.firstName} {contact.lastName}</b><small>{contact.jobTitle || (contact.isPrimary ? 'Contacto principal' : 'Contacto')}</small><em>{contact.email}</em></div></div>) : <p className="account360-muted">Todavía no hay contactos relacionados.</p>}{addingContact && <form className="account360-contact-form" onSubmit={addContact}><label><span>Nombre *</span><input name="firstName" required autoFocus/></label><label><span>Apellido *</span><input name="lastName" required/></label><label><span>Cargo</span><input name="jobTitle"/></label><label><span>Email</span><input name="email" type="email"/></label><label><span>Teléfono</span><input name="phone"/></label><footer><button type="button" className="ghost-btn" onClick={() => setAddingContact(false)}>Cancelar</button><button className="primary-btn">Guardar</button></footer></form>}</section><section className="card account360-location"><MapPin size={18}/><div><span>Ubicación</span><b>{[account.address, account.commune, account.city].filter(Boolean).join(', ') || 'Sin dirección registrada'}</b></div></section></aside></div>}
  </div>
}
