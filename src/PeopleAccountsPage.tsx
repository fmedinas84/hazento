...
function OrganizationDialog({ organization, labels, onClose, onUse }: { organization?: OrganizationData; labels: Labels; onClose: () => void; onUse: (organization: OrganizationData) => void }) {
  ...
  const save = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (current) await repository.organizationRepository.update(current.id, record)
    else await repository.organizationRepository.create(record)
    onClose()
  }
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="modal organization-dialog" role="dialog" aria-modal="true" aria-label={current?.name || labels.createOrganization} onMouseDown={event => event.stopPropagation()}>
    <header><div><span className="section-kicker">{labels.organization.toUpperCase()}</span><h2>{current?.name || labels.createOrganization}</h2><p>{current ? `${people.length} ${people.length === 1 ? 'persona relacionada' : 'personas relacionadas'}` : `Solo el nombre de la ${labels.organization.toLowerCase()} es obligatorio.`}</p></div><button className="icon-btn" onClick={onClose} aria-label="Cerrar"><X size={18}/></button></header>
...
export function PeopleAccountsPage({ labels, go, onCreate }: { labels: Labels; go: (page: 'account', query: { id: string }) => void; onCreate: () => void }) {
  const repository = useRepositories()
  ...
  const [organizationId, setOrganizationId] = useState<string | null>(requestedOrganizationId)
  ...
  const people = [...repository.accounts.filter(person => (status === 'Todos' || person.status === status.slice(0, -1) || person.status === status) && normalizeSearchText(`${person.name} ${person.email || ''} ${organizationById.get(person.organizationId || '')?.name || ''}`).includes(normalized))].sort((left, right) => ascending ? left.name.localeCompare(right.name) : right.name.localeCompare(left.name))
  const organizations = repository.organizations.filter(organization => normalizeSearchText(organization.name).includes(normalized))
  const selectedOrganization = repository.organizations.find(organization => organization.id === organizationId)

  return <>
    <div className="page-header mobile-labeled-action"><div><h1>{labels.accounts}</h1><p>{view === 'people' ? `Cada ${labels.account.toLowerCase()} es una persona; la ${labels.organization.toLowerCase()} es opcional.` : `${labels.organizations} relacionadas con tus personas, sin dominar la relación.`}</p></div><button className="primary-btn" onClick={() => view === 'people' ? onCreate() : setCreatingOrganization(true)}><Plus size={16}/>{view === 'people' ? labels.createAccount.replace(/^Nuev[oa]\s/, 'Crear ') : labels.createOrganization.replace(/^Nuev[oa]\s/, 'Crear ')}</button></div>
    ...
    {view === 'people' ? <div className="table-card card people-table"><table><thead><tr><th>Persona</th><th>{labels.organizationList}</th><th>Rol</th><th>Estado</th><th>Última actividad</th><th>Próxima actividad</th><th>Ingresos</th><th>Pendiente</th><th/></tr></thead><tbody>{people.map(person => { const organization = organizationById.get(person.organizationId || ''); return <tr key={person.id}><td><div className="person-cell"><span className="avatar" style={{ background: person.color }}>{person.initials}</span><div><button className="table-link" onClick={() => go('account', { id: person.id })}>{person.name}</button><span>{person.email || 'Sin email'}</span></div></div></td><td data-label={labels.organizationList}><b>{organization?.name || 'Independiente'}</b></td><td data-label="Rol">{person.role || (organization ? 'Sin cargo' : '—')}</td><td data-label="Estado"><span className={`badge badge-${person.status.toLowerCase()}`}>{person.status}</span></td><td>{person.last}</td><td>{person.next}</td><td className="number">{person.income}</td><td className={person.pending === '$0' ? 'muted' : 'pending-number'}>{person.pending}</td><td><button className="icon-row-action" aria-label={`Abrir ${person.name}`} onClick={() => go('account', { id: person.id })}><ChevronRight size={16}/></button></td></tr> })}</tbody></table>{!people.length && <div className="empty-state"><h3>No encontramos {labels.accounts.toLowerCase()}</h3><p>Prueba otra búsqueda o crea tu primer {labels.account.toLowerCase()}.</p><button className="primary-btn" onClick={onCreate}>{labels.createAccount}</button></div>}</div> : <div className="table-card card organization-list"><table><thead><tr><th>{labels.organization}</th><th>Personas relacionadas</th><th>RUT</th><th>Ciudad</th><th/></tr></thead><tbody>{organizations.map(organization => { const count = repository.accounts.filter(person => person.organizationId === organization.id).length; return <tr key={organization.id}><td><div className="organization-list-name"><span><Building2 size={17}/></span><button className="table-link" onClick={() => setOrganizationId(organization.id)}>{organization.name}</button></div></td><td>{count} {count === 1 ? 'persona' : 'personas'}</td><td>{organization.taxId || 'Pendiente'}</td><td>{organization.city || '—'}</td><td><button className="icon-row-action" aria-label={`Abrir ${organization.name}`} onClick={() => setOrganizationId(organization.id)}><ChevronRight size={16}/></button></td></tr> })}</tbody></table>{!organizations.length && <div className="empty-state"><h3>No hay {labels.organizations.toLowerCase()}</h3><p>Crea una {labels.organization.toLowerCase()} cuando una persona la represente.</p><button className="primary-btn" onClick={() => setCreatingOrganization(true)}>{labels.createOrganization}</button></div>}</div>}
