...
import { AppointmentReminderPanel } from './AppointmentReminders'
...
type Navigate = (page: PlanningPage, query?: Record<string, string>) => void
...
function PrestationDialog({ record, labels, go, onClose }: { record: PrestationData; labels: Labels; go: Navigate; onClose: () => void }) {
  ...
  return <Dialog title={editing ? `Editar ${labels.prestation.toLowerCase()}` : record.name} subtitle={`${record.account} · ${record.date}`} onClose={onClose}>
    {editing ? <form onSubmit={save}><div className="form-grid single">
      ...
      <div className="status-actions">{labels.prestationStatuses.map(status => status !== record.status && <button key={status} onClick={async () => await repositories.updatePrestation(record.id, { status })}>{status === labels.completedStatus && <Check size={16}/>} {status}</button>)}</div>
...
function LegacyMonthCalendar({ events, anchor, labels, onOpen, openRequestIds }: { events: CalendarEvent[]; anchor: Date; labels: Labels; onOpen: (event: CalendarEvent) => void; openRequestIds: Set<string> }) {
...
function MonthCalendar({ events, anchor, labels, onOpen, openRequestIds }: { events: CalendarEvent[]; anchor: Date; labels: Labels; onOpen: (event: CalendarEvent) => void; openRequestIds: Set<string> }) {
...
function WeekCalendar({ events, anchor, dayOnly, onOpen, openRequestIds }: { events: CalendarEvent[]; anchor: Date; dayOnly: boolean; onOpen: (event: CalendarEvent) => void; openRequestIds: Set<string> }) {
  const days = Array.from({ length: dayOnly ? 1 : 5 }, (_, index) => new Date(anchor.getFullYear(), anchor.getMonth(), anchor.getDate() + index))
  const today = demoToday()
  return <div className={cls('calendar card scheduling-calendar', dayOnly && 'calendar-día')}><div className="calendar-head"><span/>{days.map(day => <span className={day.toDateString() === today.toDateString() ? 'today' : ''} key={day.toISOString()}>{new Intl.DateTimeFormat('es-CL', { weekday: 'short' }).format(day).replace('.', '')}<b>{day.getDate()}</b></span>)}</div><div className="calendar-body"><div className="time-column">{hours.map(hour => <span key={hour}>{hour}</span>)}</div>{days.map(day => <div className="day-column" key={day.toISOString()}>{hours.map(hour => <i key={hour}/>)}{events.filter(event => { const date = new Date(event.timestamp); return date.getFullYear() === day.getFullYear() && date.getMonth() === day.getMonth() && date.getDate() === day.getDate() && date.getHours() >= 8 && date.getHours() <= 21 }).map(event => { const date = new Date(event.timestamp); const top = `calc(var(--calendar-hour-height) * ${(date.getHours() * 60 + date.getMinutes() - 480) / 60})`; return <button className={cls('cal-event', event.kind === 'prestation' ? 'violet' : 'blue', event.source === 'prestation_follow_up' && 'note-event')} style={{ top, height: 50 }} onClick={() => onOpen(event)} key={event.id}><b>{String(date.getHours()).padStart(2,'0')}:{String(date.getMinutes()).padStart(2,'0')}</b>{event.kind === 'prestation' && openRequestIds.has(event.recordId) && <i className="calendar-payment-indicator" title="Solicitud de pago abierta"><CircleDollarSign size={12}/></i>}<span>{event.source === 'prestation_follow_up' ? 'Seguimiento' : event.title}</span><small>{event.accountName}</small></button> })}</div>)}</div></div>
}

function CalendarPanel({ labels, onCreate, onSelectPrestation, onSelectActivity }: { labels: Labels; onCreate: () => void; onSelectPrestation: (id: string) => void; onSelectActivity: (id: string) => void }) {
  const repositories = useRepositories()
  ...
  const openRequestIds = new Set(repositories.paymentRequests.filter(request => request.status === 'Pendiente' && request.originPrestationId).map(request => request.originPrestationId as string))
...
function TimelinePanel({ labels, go, onSelectPrestation }: { labels: Labels; go: Navigate; onSelectPrestation: (id: string) => void }) {
  const repositories = useRepositories()
