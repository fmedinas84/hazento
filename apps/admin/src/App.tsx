...
const countryNames: Record<string, string> = { CL: 'Chile', PE: 'Perú', PY: 'Paraguay' }
...
function Login() {
  ...
  return <main className="login-page"><section className="login-card">
    <div className="brand-mark"><img src="/images/hazento-isotype.png" alt="Hazento" /></div><span className="eyebrow">HAZENTO ADMIN</span><h1>Acceso administrativo</h1>
...
function UserDetailPanel({ user, close }: { user: UserSummary; close: () => void }) {
  ...
  return <div className="drawer-backdrop" role="presentation" onMouseDown={(e) => { if (e.currentTarget === e.target) close() }}><aside className="drawer" role="dialog" aria-modal="true" aria-labelledby="user-detail-title"><button className="icon-button" onClick={close} aria-label="Cerrar ficha"><ArrowLeft /></button>{state.status === 'loading' && <Skeleton lines={10} />}{state.status === 'error' && <ErrorState message={state.message} retry={load} />}{state.data && <><span className="eyebrow">FICHA ADMINISTRATIVA</span><h2 id="user-detail-title">{state.data.name}</h2><p className="muted">{state.data.email}</p><div className="detail-grid"><Detail title="Cuenta" values={[['Estado', healthNames[state.data.health]], ['Alta', formatDate(state.data.registeredAt)], ['Último login', formatDate(state.data.lastSignInAt, true)], ['Última actividad', formatDate(state.data.lastActivityAt, true)]]} /><Detail title="Workspace" values={[['Nombre', state.data.workspaceName], ['Profesión', verticalNames[state.data.vertical] ?? state.data.vertical], ['País de operación', countryNames[state.data.countryCode] ?? state.data.countryCode], ['Creado', formatDate(state.data.workspaceCreatedAt)]]} /><Detail title="Suscripción" values={[['Plan', state.data.subscription.plan.toUpperCase()], ['Estado', state.data.subscription.status], ['Inicio', formatDate(state.data.subscription.startedAt)], ['Próxima renovación', formatDate(state.data.subscription.nextPaymentAt)], ['Proveedor', state.data.subscription.provider ?? 'No asociado']]} /><Detail title="Uso" values={[['Clientes', String(state.data.clients)], ['Atenciones', String(state.data.prestations)], ['Oportunidades', String(state.data.opportunities)], ['Solicitudes', String(state.data.paymentRequests)], ['Pagos', String(state.data.payments)]]} /></div><section className="detail-card"><h3>Activación</h3><div className="milestones">{Object.entries({ 'Creó cuenta': state.data.milestones.accountCreatedAt, 'Primer cliente': state.data.milestones.firstClientAt, 'Primera atención': state.data.milestones.firstPrestationAt, 'Primer pago': state.data.milestones.firstPaymentAt }).map(([label, value]) => <div key={label}><i className={value ? 'done' : ''} /><span>{label}<small>{value ? formatDate(value) : 'Pendiente'}</small></span></div>)}</div></section></>}</aside></div>
}
