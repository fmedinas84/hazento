...
Atenciones y Clases comparten una única plantilla de email con nombre, tipo de cita, fecha, hora y la dirección configurada en Configuración → Negocio. La vista previa de Configuración → Recordatorios reutiliza esa misma plantilla; nunca incluye seguimiento ni información clínica. El remitente futuro será `Nombre profesional vía Hazento <recordatorios@hazento.cl>` y queda preparado `replyTo`.
